...
const MODERATION_ACTIONS = ['warn', 'timeout', 'kick', 'ban', 'unban', 'purge', 'delete_message'];
analyticsRouter.get('/', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const server = await getOrCreateServerRow(req.params.serverId);
    const since7d = new Date(Date.now() - 7 * 86_400_000).toISOString();
    const [ticketsTotal, ticketsOpen, ticketsClosed7d, warningsTotal, giveawaysActive, memberJoins7d, memberLeaves7d,] = await Promise.all([
    ...
    await Promise.all(MODERATION_ACTIONS.map(async (action) => {
