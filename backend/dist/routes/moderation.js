...
import { assertCanModerate, banMember, deleteMessage, fetchRecentMessageIds, kickMember, purgeMessages, timeoutMember, unbanMember, } from '../lib/discord.js';
export const moderationRouter = Router({ mergeParams: true });
async function applyWarningAutomations(serverRowId, guildId, discordUserId, moderatorDiscordId) {
    const automations = await getServerSetting(serverRowId, 'warning_automations', []);
...
moderationRouter.post('/warn', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    ...
    const { data: warning, error } = await supabase
        ...
        .single();
    if (error || !warning)
        throw error ?? new Error('Failed to record warning');
    ...
    const escalation = await applyWarningAutomations(server.id, req.params.serverId, body.discordUserId, req.session.discordId);
    res.status(201).json({ warning, escalation });
}));
moderationRouter.get('/warnings', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const server = await getOrCreateServerRow(req.params.serverId);
...
moderationRouter.delete('/warnings/:warningId', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const server = await getOrCreateServerRow(req.params.serverId);
...
moderationRouter.post('/timeout', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const body = z.object({ ...targetSchema.shape, minutes: z.number().int().positive().max(40320) }).parse(req.body);
...
moderationRouter.post('/kick', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
...
moderationRouter.post('/ban', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const body = z
...
moderationRouter.post('/unban', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const body = targetSchema.parse(req.body);
...
moderationRouter.post('/purge', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const body = purgeSchema.parse(req.body);
    const ids = await fetchRecentMessageIds(body.channelId, body.amount);
    if (ids.length > 0)
        await purgeMessages(body.channelId, ids);
    const server = await getOrCreateServerRow(req.params.serverId);
    await recordModerationAction(server.id, {
        action: 'purge',
        moderatorDiscordId: req.session.discordId,
...
moderationRouter.delete('/messages/:channelId/:messageId', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
...
moderationRouter.get('/logs', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const server = await getOrCreateServerRow(req.params.serverId);
