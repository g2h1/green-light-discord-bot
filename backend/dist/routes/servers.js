...
import { canManageGuild, fetchBotGuildIds, fetchGuildChannelCount, fetchGuildDetail, fetchGuildRoleCount, fetchUserGuilds, } from '../lib/discord.js';
import { env } from '../lib/env.js';
import { supabase } from '../lib/supabase.js';
import { HttpError } from '../middleware/errorHandler.js';
export const serversRouter = Router();
serversRouter.get('/', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (installedServers.length > 0) {
        await supabase
            .from('servers')
            .upsert(installedServers.map((s) => ({ discord_guild_id: s.id, name: s.name, icon: s.icon })), { onConflict: 'discord_guild_id' });
    }
    res.json({ servers });
}));
serversRouter.get('/:serverId/overview', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
