...
settingsRouter.get('/automod', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
...
settingsRouter.put('/automod', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
...
settingsRouter.get('/log-channels', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const server = await getOrCreateServerRow(req.params.serverId);
    const value = await getServerSetting(server.id, 'log_channels', {});
    res.json({ logChannels: value });
}));
settingsRouter.put('/log-channels', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const body = logChannelsSchema.parse(req.body);
    const server = await getOrCreateServerRow(req.params.serverId);
    await setServerSetting(server.id, 'log_channels', body);
    res.json({ logChannels: body });
}));
const warningAutomationSchema = z.array(z.object({
    threshold: z.number().int().positive(),
    action: z.enum(['timeout', 'ban']),
    durationMinutes: z.number().int().positive().optional(),
}));
settingsRouter.get('/warning-automations', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const server = await getOrCreateServerRow(req.params.serverId);
    const value = await getServerSetting(server.id, 'warning_automations', []);
    res.json({ automations: value });
}));
settingsRouter.put('/warning-automations', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
...
settingsRouter.get('/welcome', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const server = await getOrCreateServerRow(req.params.serverId);
    const value = await getServerSetting(server.id, 'welcome', WELCOME_DEFAULTS);
    res.json({ welcome: value });
}));
settingsRouter.put('/welcome', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
...
settingsRouter.get('/strict-channels', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
    const server = await getOrCreateServerRow(req.params.serverId);
    const value = await getServerSetting(server.id, 'strict_channels', []);
    res.json({ strictChannels: value });
}));
settingsRouter.put('/strict-channels', requireAuth, requireServerAccess, asyncHandler(async (req, res) => {
