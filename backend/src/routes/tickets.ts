...
const BUTTON_COLOR_STYLES = { green: 3, gray: 2, red: 4, blue: 1 } as const
...
function buildPanelEmbed(body: z.infer<typeof panelSchema>): EmbedPayload {
  return {
    title: body.title,
...
async function postOrUpdatePanelMessage(
  channelId: string,
  panelId: string,
  embed: EmbedPayload,
  config: z.infer<typeof panelConfigSchema>,
  ...
  if (existingMessageId) {
    ...
    if (res.ok) return { id: existingMessageId }
...
ticketsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (error || !panel) throw error ?? new Error('Failed to create panel')
...
ticketsRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!existing) throw new HttpError(404, 'Panel not found')
    ...
    const oldConfig = (existing.config ?? {}) as { imageUrl?: string; thumbnailUrl?: string }
    const staleUrls = [
    ...
    if (body.disabled) {
      ...
      const sent = await postOrUpdatePanelMessage(
        body.channelId,
        req.params.panelId,
        embed,
        config,
        body.channelId === existing.channel_id ? existing.message_id : null,
      )
      messageId = sent.id
    }
    ...
    if (error || !panel) throw error ?? new Error('Failed to update panel')
...
async function loadTicket(serverRowId: string, ticketId: string) {
...
ticketsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (error || !data) throw error ?? new Error('Failed to claim ticket')
...
ticketsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (error || !data) throw error ?? new Error('Failed to close ticket')
    res.json({ ticket: data })
  }),
)
...
ticketsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (error || !data) throw error ?? new Error('Failed to reopen ticket')
    res.json({ ticket: data })
  }),
)
