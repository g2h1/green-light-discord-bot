...
import { sendChannelMessage, type EmbedPayload } from '../lib/discord.js'
import { supabase } from '../lib/supabase.js'

export const giveawaysRouter = Router({ mergeParams: true })

function giveawayEmbed(prize: string, winnersCount: number, endsAt: string, requiredRoleId?: string): EmbedPayload {
  return {
    title: `🎉 ${prize}`,
    description: [
...
giveawaysRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (error || !giveaway) throw error ?? new Error('Failed to create giveaway')
...
giveawaysRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (error) throw error
    if (!giveaway) throw new HttpError(404, 'Giveaway not found')
...
giveawaysRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (error || !data) throw error ?? new Error('Failed to cancel giveaway')
    res.json({ giveaway: data })
  }),
)

async function pickWinners(giveawayId: string, count: number, excludeIds: string[] = []) {
...
giveawaysRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (error) throw error
    if (!giveaway) throw new HttpError(404, 'Giveaway not found')
    if (giveaway.status !== 'ended') throw new HttpError(400, 'Only an ended giveaway can be rerolled')

    const newWinners = await pickWinners(giveaway.id, giveaway.winners_count, giveaway.winner_discord_ids)
    if (newWinners.length === 0) throw new HttpError(400, 'No remaining entrants to reroll from')
    ...
    await sendChannelMessage(giveaway.channel_id, {
      content: `🔁 Reroll for **${giveaway.prize}**: ${newWinners.map((id: string) => `<@${id}>`).join(', ')}`,
    })
