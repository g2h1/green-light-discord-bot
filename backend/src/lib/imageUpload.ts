 ...
 */
export async function deletePanelImageIfOwned(url: string) {
  const marker = `/storage/v1/object/public/${BUCKET}/`
  const idx = url.indexOf(marker)
  if (idx === -1) return // not one of our uploaded assets (e.g. a manually-entered external URL) — leave it alone
  const path = decodeURIComponent(url.slice(idx + marker.length))
  await supabase.storage.from(BUCKET).remove([path]).catch(() => undefined)
}

/** Called only when the whole panel row is deleted — safe to remove everything under its own path. */
export async function deleteAllPanelImages(panelId: string) {
