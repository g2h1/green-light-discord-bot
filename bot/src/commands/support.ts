import { ChannelType, SlashCommandBuilder, type ChatInputCommandInteraction, type GuildMember } from 'discord.js'
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addChannelOption((opt) =>
        opt.setName('room').setDescription('Destination voice channel').setRequired(true).addChannelTypes(ChannelType.GuildVoice),
      ),
  )
  .addSubcommand((sub) =>
    sub
...
async function requireStaff(interaction: ChatInputCommandInteraction, staffRoleIds: string[]) {
  const member = interaction.member
  if (!member || !('roles' in member)) return false
  return hasAnyRole(member as GuildMember, staffRoleIds)
}
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (sub === 'claim') {
    ...
    if (!result.ok) {
      await interaction.editReply(
        result.reason === 'empty' ? t(config.locale, 'support.noWaitingUsers') : t(config.locale, 'support.roomFull'),
      )
      return
    }
  ...
  if (sub === 'close') {
    ...
    await interaction.editReply(
      closed ? `Closed the support session for <@${targetUser.id}>.` : t(config.locale, 'support.notInSession'),
    )
    return
  }
  ...
  if (sub === 'status') {
    ...
    await interaction.editReply(
      `Waiting: ${waiting.length}\nFree rooms: ${freeRooms.length}/${config.roomChannelIds.length}\nWaiting channel: ${
        config.waitingChannelId ? `<#${config.waitingChannelId}>` : 'not configured'
      }`,
    )
  }
}
