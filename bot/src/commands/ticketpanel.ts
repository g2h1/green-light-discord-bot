...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (sub === 'set-image') {
    const slot = interaction.options.getString('slot', true) as Slot
    const attachment = interaction.options.getAttachment('image', true)

    const previousPath = panel[SLOT_COLUMN[slot]] as string | null
    try {
      ...
      if (previousPath && previousPath !== path) await removePanelAsset(previousPath)
  ...
  if (sub === 'remove-image') {
    const slot = interaction.options.getString('slot', true) as Slot
    const path = panel[SLOT_COLUMN[slot]] as string | null
    if (!path) {
  ...
  if (sub === 'preview') {
    ...
    if (mainUrl) embed.setImage(mainUrl)
    ...
    if (bannerUrl) await interaction.followUp({ content: bannerUrl, ephemeral: true })
  }
}
