...
async function checkAntiRaid(guild: import('discord.js').Guild) {
  ...
  if (timestamps.length === RAID_JOIN_THRESHOLD) {
    await logEvent(
      ...
      'security',
      'anti_raid',
      `Possible raid detected: ${RAID_JOIN_THRESHOLD}+ members joined within ${RAID_WINDOW_MS / 1000}s.`,
      {},
    )
  }
}

export function registerGuildLogging(client: Client) {
  ...
  client.on('messageDelete', (message) => {
    ...
    void logEvent(
      ...
      'message',
      'message_delete',
      `Message by ${message.author?.tag ?? 'unknown'} deleted in #${'name' in message.channel ? message.channel.name : message.channelId}.`,
      { authorId: message.author?.id, channelId: message.channelId, content: message.content?.slice(0, 500) },
    )
  })
