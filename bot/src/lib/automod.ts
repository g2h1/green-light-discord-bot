...
const recentMessages = new Map<string, number[]>() // userId -> timestamps, for anti-spam/flood
const lastMessageByUser = new Map<string, { content: string; count: number }>() // userId -> last content + streak
const URL_REGEX = /https?:\/\/\S+/i

function violation(config: AutomodConfig, message: Message): string | null {
  const content = message.content

  if (config.linkFilter && URL_REGEX.test(content)) return 'link_filter'

  if (config.badWordFilter && config.bannedWords.length > 0) {
    const lower = content.toLowerCase()
    if (config.bannedWords.some((w) => w && lower.includes(w.toLowerCase()))) return 'bad_word_filter'
  }
  ...
  if (config.capsDetection && content.length >= CAPS_MIN_LENGTH) {
    ...
    if (letters.length > 0 && upper.length / letters.length >= CAPS_RATIO_THRESHOLD) return 'caps_detection'
  }
  ...
  if (config.duplicateMessage) {
    ...
    if (last && last.content === content) {
      ...
      if (last.count >= DUPLICATE_THRESHOLD) return 'duplicate_message'
    } else {
  ...
  if (config.antiSpam || config.antiFlood) {
    ...
    if (timestamps.length > SPAM_MESSAGE_LIMIT) return config.antiFlood ? 'anti_flood' : 'anti_spam'
  }
...
export async function checkAutomod(message: Message) {
  ...
  await logEvent(
    ...
    'security',
    reason,
    `AutoMod (${reason.replace('_', ' ')}) removed a message from ${message.author.tag} in #${'name' in message.channel ? message.channel.name : message.channelId}.`,
