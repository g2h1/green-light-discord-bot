...
export async function logEvent(guild, category, eventType, summary, metadata = {}) {
    ...
    const { data: setting } = await supabase
        ...
        .eq('key', 'log_channels')
        .maybeSingle();
    const channelId = setting?.value?.[CATEGORY_TO_CHANNEL_KEY[category]];
    if (!channelId)
        return;
