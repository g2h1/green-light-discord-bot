...
function actionButtons(status, config) {
    ...
    if (status !== 'closed') {
        if (config.enableClaim) {
            ...
            if (config.claimButtonEmoji)
                b.setEmoji(config.claimButtonEmoji);
            row.addComponents(b);
        }
        if (config.enableClose) {
            ...
            if (config.closeButtonEmoji)
                b.setEmoji(config.closeButtonEmoji);
            row.addComponents(b);
        }
    }
    else {
...
function languageButtons(ticketId) {
    const row = new ActionRowBuilder();
    row.addComponents(new ButtonBuilder().setCustomId(`ticket_lang:${ticketId}:ar`).setLabel('🇪🇬 العربية').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`ticket_lang:${ticketId}:en`).setLabel('🇺🇸 English').setStyle(ButtonStyle.Secondary));
    return row;
}
function ratingButtons() {
    const row = new ActionRowBuilder();
    for (let stars = 1; stars <= 5; stars++) {
        row.addComponents(new ButtonBuilder().setCustomId(`ticket_rate:${stars}`).setLabel('★'.repeat(stars)).setStyle(ButtonStyle.Secondary));
    }
    return row;
}
async function countOpenTickets(serverId, openerDiscordId) {
...
function fillTicketVariables(text, vars) {
    return text
...
export async function handleTicketOpen(interaction, panelId, categoryIndex) {
    ...
    if (openCount >= config.maxOpenTicketsPerUser) {
        await interaction.editReply(`You already have ${openCount} open ticket(s), which is the limit for this server (${config.maxOpenTicketsPerUser}).`);
        return;
    }
    ...
    if (config.welcomeEnabled) {
        const embed = new EmbedBuilder()
            ...
            .setDescription(config.welcomeEmbedDescription
...
async function createTicketChannel(guild, name, openerId, parentId) {
...
export async function handleTicketClaim(interaction) {
    ...
    if (!ticket)
        return interaction.editReply(t('en', 'ticket.notATicketChannel'));
...
async function postTranscript(guild, ticket, config) {
    if (!config?.enableTranscript || config.transcriptBehavior === 'none')
        return;
...
export async function handleTicketClose(interaction) {
    ...
    if (interaction.guild)
        await postTranscript(interaction.guild, ticket, config);
    ...
    if (interaction.guild) {
        ...
        if (config?.deleteOnClose && !config.archiveInsteadOfDelete) {
            ...
            if (ch)
                setTimeout(() => ch.delete().catch(() => undefined), 5000);
        }
    }
}
...
export async function handleTicketRate(interaction, stars) {
    await interaction.deferReply({ ephemeral: true });
    const ticket = await loadTicketByChannel(interaction.channelId);
    if (!ticket)
        return interaction.editReply(t('en', 'ticket.notATicketChannel'));
    await supabase.from('ticket_ratings').upsert({ ticket_id: ticket.id, stars: Number(stars), rated_by_discord_id: interaction.user.id }, { onConflict: 'ticket_id' });
    await interaction.editReply(t(ticket.language === 'ar' ? 'ar' : 'en', 'ticket.ratingThanks', { stars }));
}
export async function logTicketMessage(channelId, authorId, authorUsername, content) {
