...
function isExempt(message, rule) {
    ...
    if (!member)
        return true; // fail open: can't verify, don't punish
    ...
    if (member.permissions.has(PermissionFlagsBits.ModerateMembers))
        return true;
    if (rule.exemptRoleIds.some((roleId) => member.roles.cache.has(roleId)))
        return true;
    return false;
}
/** Returns true if this message was a strict-channel violation (already handled). */
export async function checkStrictChannel(message) {
    ...
    if (!server)
        return false;
    const rules = await getServerSetting(server.id, 'strict_channels', []);
    const rule = rules.find((r) => r.channelId === message.channelId && r.enabled);
    if (!rule)
        return false;
    if (isExempt(message, rule))
        return false;
    await message.delete().catch(() => undefined);
    const botMember = message.guild.members.me;
    const canBan = botMember &&
