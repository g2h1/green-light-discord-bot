...
export async function getSupportConfig(serverRowId) {
...
async function renumberQueue(serverId, waitingChannelId) {
    ...
    if (!waiting)
        return;
    await Promise.all(waiting.map((row, index) => supabase.from('support_queue').update({ position: index + 1 }).eq('id', row.id)));
}
/** Adds a user to the waiting queue. Idempotent — a duplicate join is reported, not re-inserted. */
export async function joinQueue(guild, userId, waitingChannelId) {
    ...
    const { data: inserted, error } = await supabase
        ...
        .single();
    if (error || !inserted)
        throw error ?? new Error('Failed to join support queue');
...
export async function leaveQueue(guild, userId) {
    const server = await getOrCreateServerRow(guild);
...
export async function listWaiting(serverRowId) {
...
function findFreeRoom(guild, roomChannelIds) {
...
export async function claimNext(guild, staffId, config, targetUserId) {
    const server = await getOrCreateServerRow(guild);
...
export async function moveSession(guild, userId, room) {
...
export async function closeSession(guild, userId) {
    const server = await getOrCreateServerRow(guild);
    const { data: entry } = await supabase
        .from('support_queue')
        .select('id')
...
export function announceQueuePosition(locale, position) {
...
export async function ensureWaitingChannelPermissions(guild, channelId) {
